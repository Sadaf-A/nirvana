  ...
  const products = [
    {
      ...
      price: 125,
      description: "Beautiful vase handcrafted from recycled glass bottles. Come as you are, eco-style.",
      category: "home-decor",
      seller: "GreenArtisan",
    ...
    {
      id: 2,
      title: "Upcycled Plant Stand",
      price: 240,
      description: "Stylish plant stand made from recycled wood and metal. Smells like eco-spirit.",
      category: "furniture",
      seller: "EcoFurnish",
      image: "/api/placeholder/400/400"
    },
    {
      id: 3,
      title: "Recycled Fabric Tote",
      price: 85,
      description: "Durable tote bag made from recycled fabrics. Lithium-free and built to last.",
    ...
    },
    {
      ...
      price: 175,
      description: "Creative wall art made from upcycled bottle caps. Heart-shaped box of sustainability.",
      category: "home-decor",
      seller: "CapCreations",
      image: "/api/placeholder/400/400"
    },
    {
      ...
      price: 195,
      description: "Unique lamp crafted from beach driftwood. Lights out, but eco-friendly.",
      category: "home-decor",
      seller: "OceanFinds",
      image: "/api/placeholder/400/400"
    },
    {
      ...
      price: 320,
      description: "Rustic coffee table made from reclaimed pallets. Territorial pissings not included.",
      category: "furniture",
      seller: "WoodWorks",
      image: "/api/placeholder/400/400"
    },
    {
      id: 7,
      title: "Vinyl Record Bowl",
      price: 65,
      description: "Decorative bowl made from old vinyl records. For those about to recycle, we salute you.",
      category: "home-decor",
      seller: "MusicUpcycle",
      image: "/api/placeholder/400/400"
    },
    {
      ...
      price: 45,
      description: "Handmade journal from 100% recycled paper. All apologies for cutting trees are unnecessary.",
      category: "accessories",
      seller: "PaperTrail",
      image: "/api/placeholder/400/400"
    }
              ...
              <div className="flex flex-col md:flex-row justify-between gap-4 mb-6">
                <div className="flex gap-2 overflow-x-auto pb-2">
                  ...
                  <button 
                    ...
                    className={`${selectedCategory === 'home-decor' ? 
                      `${isDarkMode ? 'bg-white/5 backdrop-blur-lg border border-white/10' : 'bg-[#9d81ff]/10 border border-[#9d81ff]/20'} ${textColor}` : 
                      `${textColorMuted} ${buttonHover}`} px-4 py-2 rounded-xl text-sm transition duration-300`}
                  >
                    Home Decor
                  ...
                  <button 
                    ...
                    className={`${selectedCategory === 'furniture' ? 
                      `${isDarkMode ? 'bg-white/5 backdrop-blur-lg border border-white/10' : 'bg-[#9d81ff]/10 border border-[#9d81ff]/20'} ${textColor}` : 
                      `${textColorMuted} ${buttonHover}`} px-4 py-2 rounded-xl text-sm transition duration-300`}
                  >
                    Furniture
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {filteredProducts.length > 0 ? (
                  filteredProducts.map((product) => (
                    <div key={product.id} className={`${cardBg} rounded-xl overflow-hidden hover:shadow-lg ${isDarkMode ? 'hover:shadow-[#9d81ff]/20' : 'hover:shadow-[#9d81ff]/30'} transition duration-300 group`}>
                      <div className="aspect-square overflow-hidden">
                        <img src={product.image} alt={product.title} className="object-cover w-full h-full group-hover:scale-105 transition-transform duration-700" />
                      </div>
                      <div className="p-4">
                        <div className="flex justify-between items-start mb-2">
                          <h3 className={`${textColor} font-medium text-lg`}>{product.title}</h3>
                        ...
                        <p className={`${textColorMuted} text-sm mb-4`}>{product.description}</p>
                        <div className="flex justify-between items-center">
                          <div className="flex items-center gap-1 text-xs ${textColorMuted}">
                            <span>By</span>
                            <span className={textColor}>{product.seller}</span>
                          </div>
              ...
              {filteredProducts.length > 0 && (
                <div className="flex justify-center mt-8">
                  <div className="flex gap-2">
                    <button className={`w-10 h-10 flex items-center justify-center rounded-lg ${isDarkMode ? 'bg-white/5 backdrop-blur-lg border border-white/10' : 'bg-[#9d81ff]/10 border border-[#9d81ff]/20'} ${textColor}`}>1</button>
                    <button className={`w-10 h-10 flex items-center justify-center rounded-lg ${buttonHover} ${textColorMuted}`}>2</button>
                    <button className={`w-10 h-10 flex items-center justify-center rounded-lg ${buttonHover} ${textColorMuted}`}>3</button>
                    <button className={`w-10 h-10 flex items-center justify-center rounded-lg ${buttonHover} ${textColorMuted}`}>
            ...
            <div className="mb-12">
              <div className={`${cardBg} rounded-xl p-6 md:p-8 max-w-3xl mx-auto transition duration-300`}>
                ...
                <form className="space-y-6">
                  <div>
                    ...
                    <div className="relative group transition-all duration-300">
                      <input 
                        type="text" 
                        id="itemTitle" 
                        className={`w-full ${inputBg} rounded-xl px-4 py-4 ${textColor} focus:outline-none focus:border-[#9d81ff] focus:ring-1 focus:ring-[#9d81ff] transition-all duration-300 placeholder-transparent peer`} 
                        placeholder="Enter item name"
                        required 
                      />
                      <label 
                        htmlFor="itemTitle" 
                        className={`absolute left-4 -top-2.5 px-1 text-sm ${textColorMuted} ${isDarkMode ? 'bg-[#121212]' : 'bg-gray-100'} peer-placeholder-shown:text-base peer-placeholder-shown:top-4 peer-focus:-top-2.5 peer-focus:text-sm peer-focus:text-[#9d81ff] transition-all duration-200`}
                      >
                        Enter item name
                      </label>
                    </div>
                  ...
                  <div>
                    <label htmlFor="itemPrice" className={`block ${textColor} text-sm font-medium mb-2`}>Price (EcoCoins)</label>
                    <div className="relative group transition-all duration-300">
                      <input 
                        type="number" 
                        id="itemPrice" 
                  ...
                  <div>
                    <label htmlFor="itemDescription" className={`block ${textColor} text-sm font-medium mb-2`}>Description</label>
                    <textarea
                      id="itemDescription"
                      rows="4"
                      className={`w-full ${inputBg} rounded-xl px-4 py-4 ${textColor} focus:outline-none focus:border-[#9d81ff] focus:ring-1 focus:ring-[#9d81ff] transition-all duration-300`}
                      placeholder="Tell us about your item and how it was upcycled..."
                      required
                    ></textarea>
                    <p className={`text-xs ${textColorMuted} mt-2`}>Add a Nirvana reference for bonus EcoCoins!</p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className={`border-2 border-dashed ${isDarkMode ? 'border-white/10' : 'border-gray-300'} rounded-xl p-8 text-center`}>
                      ...
                      <p className={`${textColorMuted} text-sm mb-2`}>Drag & drop images here, or <span className={`${textColor} font-medium cursor-pointer hover:underline`}>click to upload</span></p>
              ...
              <div className={`${cardBg} rounded-xl p-6 md:p-8 max-w-3xl mx-auto transition duration-300`}>
                ...
                <div className={`${cardBg} rounded-xl p-4 mb-4`}>
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className={`${textColor} font-medium text-lg`}>Recycled Glass Vase</h3>
                      <p className={`${textColorMuted} text-sm`}>Listed 3 days ago</p>
                    </div>
                    <div className="flex items-center gap-4">
