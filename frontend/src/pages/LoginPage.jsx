...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://nirvana-454711.el.r.appspot.com/login', {
        email,
        password,
      });
    ...
    <div className="flex flex-col w-full md:w-1/2 max-w-md">
      <div className="mb-8">
        ...
        <p className="text-white/60 text-sm mb-6">
          Your journey to a cleaner planet continues here
        </p>
      </div>
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <div 
                ...
                className={`w-10 h-6 rounded-full ${rememberMe ? 'bg-gradient-to-r from-[#9d81ff] to-[#38ef7d]' : 'bg-white/10'} cursor-pointer transition-colors duration-300`}
              >
                <div 
                  className={`w-5 h-5 rounded-full bg-white transform transition-transform duration-300 ${rememberMe ? 'translate-x-4' : 'translate-x-1'} shadow-md translate-y-0.5`}
                ></div>
              </div>
            </div>
            <label htmlFor="remember" className="text-white/60 text-sm cursor-pointer">Remember me</label>
          </div>
          <Link to="#" className="text-[#9d81ff] text-sm font-medium hover:text-[#b5a0ff] transition-colors duration-300">Forgot password?</Link>
        </div>
  ...
  >
    ...
    Apple
      ...
      <p className="text-white/60 text-center mt-8">
        ...
        <Link to="/signup" className="text-[#9d81ff] font-medium ml-1 hover:text-[#b5a0ff] transition-colors duration-300">Join Nirvana</Link>
      </p>
    </div>
  );
};

// Using the existing FactsCard you provided
const FactsCard = () => {
  return (
      ...
      <div className="mt-6 pt-6 border-t border-[#333]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <p className="text-white/60 text-xs">5,234 people recycling right now</p>
          </div>
        </div>
      </div>
    </div>
